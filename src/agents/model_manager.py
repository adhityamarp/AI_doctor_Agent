...
class ModelManager:
    ...
    def generate_analysis(self, data, system_prompt, retry_count=0):
        ...
        if provider not in self.clients:
            ...
            if retry_count == 0 and self.client_init_error:
                return {
                    "success": False,
                    "error": f"AI client initialization failed: {self.client_init_error}",
                }
        ...
        try:
            ...
            next_result = self.generate_analysis(data, system_prompt, retry_count + 1)
            if not next_result.get("success") and "error" in next_result:
                next_result["error"] = f"{model}: {str(e)} | {next_result['error']}"
            return next_result
